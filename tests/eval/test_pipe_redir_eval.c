...
int	setup_pipe_redir_prefix_input_to_wc(void **state)
{
	...
	fd = open("/tmp/pipe_redir_prefix_in.txt", O_CREAT | O_TRUNC | O_WRONLY, 0644);
	if (fd < 0)
		return (-1);
	write(fd, content, strlen(content));
	close(fd);
...
int	setup_pipe_redir_prefix_in_out(void **state)
{
	...
	fd = open("/tmp/pipe_redir_prefix_in_out_src.txt", O_CREAT | O_TRUNC | O_WRONLY,
			0644);
	if (fd < 0)
		return (-1);
	write(fd, content, strlen(content));
	...
	cmd_cat1 = create_cmd_ast("cat", NULL, 0);
	cmd_cat2 = create_cmd_ast("cat", NULL, 0);
	redir_in = create_io_file_node(LESS, "/tmp/pipe_redir_prefix_in_out_src.txt");
	redir_out = create_io_file_node(GREAT, "/tmp/pipe_redir_prefix_in_out_dest.txt");
	if (!cmd_cat1 || !cmd_cat2 || !redir_in || !redir_out)
		return (-1);
...
int	setup_pipe_redir_prefix_append(void **state)
{
	...
	cmd_echo = create_cmd_ast("echo", (char *[]){"world", NULL}, 1);
	cmd_cat = create_cmd_ast("cat", NULL, 0);
	redir_append = create_io_file_node(DGREAT, "/tmp/pipe_redir_prefix_append.txt");
	if (!cmd_echo || !cmd_cat || !redir_append)
		return (-1);
...
int	setup_pipe_redir_prefix_complex(void **state)
{
	...
	fd = open("/tmp/pipe_redir_prefix_complex_in.txt", O_CREAT | O_TRUNC | O_WRONLY,
			0644);
	if (fd < 0)
		return (-1);
	write(fd, content, strlen(content));
	...
	redir_in = create_io_file_node(LESS, "/tmp/pipe_redir_prefix_complex_in.txt");
	redir_out = create_io_file_node(GREAT, "/tmp/pipe_redir_prefix_complex_out.txt");
...
int	setup_pipe_redir_prefix_override_pipe(void **state)
{
	...
	fd = open("/tmp/pipe_redir_prefix_ignore_in.txt", O_CREAT | O_TRUNC | O_WRONLY,
			0644);
	if (fd < 0)
		return (-1);
	write(fd, content, strlen(content));
	...
	cmd_cat = create_cmd_ast("cat", NULL, 0);
	redir_in = create_io_file_node(LESS, "/tmp/pipe_redir_prefix_ignore_in.txt");
	redir_out = create_io_file_node(GREAT, "/tmp/pipe_redir_prefix_ignore_out.txt");
...
int	setup_pipe_redir_prefix_suffix_mixed(void **state)
{
	...
	fd = open("/tmp/pipe_redir_prefix_mixed_in.txt", O_CREAT | O_TRUNC | O_WRONLY,
			0644);
	if (fd < 0)
		return (-1);
	write(fd, content, strlen(content));
	...
	redir_out = create_io_file_node(GREAT, "/tmp/pipe_redir_prefix_mixed_out.txt");
